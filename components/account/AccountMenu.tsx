...
export function AccountMenu({ action, title, children }: {action?: (formData: FormData) => Promise<{ error: string | null }>, title: string, children: ReactNode}) {
  ...
  const handleAction = action && ( async (formData: FormData) => {
  ...
  return (
    <form action={handleAction} className="w-[400px] h-fit bg-white shadow rounded-xl flex flex-col p-[20px] gap-6 border-2">
