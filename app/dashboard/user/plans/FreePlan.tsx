...
export default function FreePlan({ plan, selected }: { plan: SubscriptionType, selected: boolean}) {
    ...
    return (
        <div className={`w-full h-[150px] rounded-xl shadow flex items-center justify-between p-[15px] border-2 ${selected ? "bg-gray-200" : "bg-gray-50" }`}>
            ...
            {selected ? <h1 className={`text-center ${kanit.semiBold} text-gray-800 tracking-wide text `}>Current Plan</h1> : <button className={`self-center px-[15px] py-[10px] rounded-lg shadow bg-gray-600 text-gray-50 ${kanit.semiBold} tracking-wide`}>Select</button>}
        </div>
    )
}